                ...
                <div class="card-body px-4">
                    <?php
                        if(isset($_POST['submit'])) {
                            ...
                            $prev_image = $_POST['prev_img'];
                            $image = $_FILES['file']['name'];
                            $title = $_POST['title'];
                            $content = $_POST['content'];
                            $category = $_POST['category'];

                            $sql = 'update posts set Title = ?, Category = ?, Content = ?, Image = ? where PostID = ?';
                            $stmt = $con->prepare($sql);
                            $stmt->execute(array($title, $category, $content, $image, $id));

                            if(file_exists('uploads/'.$prev_image)) {
                                unlink('uploads/'.$prev_image); ?>
                                <?php if(move_uploaded_file($_FILES['file']['tmp_name'], $target)) { ?>
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="hidden" class="form-control" id="prev" value="<?=$post['Image'];?>"  name="prev_img">
                        ...
                        <div class="form-group">
                            <button type="submit" name="submit" class="btn btn-sm btn-primary">Update</button>&nbsp;<a href="get_post.php" class="btn btn-sm btn-dark" role="button">Back</a>
                        </div>
                      </form>
