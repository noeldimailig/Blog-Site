          ...
          <div class="row">
            <?php 
              if(!isset($_POST['submit'])) {
                ...
                $sql = 'select p.PostID, p.Title, p.Category, p.Content, p.Image, p.AddedAt, p.UpdatedAt, u.FullName from posts as p inner join users as u on p.UserID = u.UserID where p.Category = ? order by p.PostID';
                $stmt = $con->prepare($sql);
                $stmt->execute([$category]);
                
              } else {
                $search = $_POST['search'];
                $sql = 'select p.PostID, p.Title, p.Category, p.Content, p.Image, p.AddedAt, p.UpdatedAt, u.FullName from posts as p inner join users as u on p.UserID = u.UserID where p.Title like ? order by p.PostID';
                $search = '%'.$search.'%';
                $stmt = $con->prepare($sql);
                $stmt->execute([$search]);
              }
